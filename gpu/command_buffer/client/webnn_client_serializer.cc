...
WebnnClientSerializer::WebnnClientSerializer(
    WebNNImplementation* client,
...
WebnnClientSerializer::~WebnnClientSerializer() = default;

size_t WebnnClientSerializer::GetMaximumAllocationSize() const {
  return transfer_buffer_->GetMaxSize();
}

void* WebnnClientSerializer::GetCmdSpace(size_t size) {
  ...
  TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("gpu.dawn"),
               "WebnnClientSerializer::GetCmdSpace", "bytes", allocation_size);
...
void WebnnClientSerializer::SetAwaitingFlush(bool awaiting_flush) {
...
void WebnnClientSerializer::Disconnect() {
  buffer_.Discard();
  if (transfer_buffer_) {
...
bool WebnnClientSerializer::Flush() {
  Commit();
  return true;
}
