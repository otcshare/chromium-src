...
WebNNCommandBufferStub::WebNNCommandBufferStub(
    GpuChannel* channel,
...
gpu::ContextResult WebNNCommandBufferStub::Initialize(
...
#else
  TRACE_EVENT0("gpu", "WebNNBufferStub::Initialize");
  UpdateActiveUrl();
  ...
  if (share_command_buffer_stub) {
    LOG(ERROR) << "Using a share group is not supported with WebNNDecoder";
    return ContextResult::kFatalFailure;
  }

  if (surface_handle_ != kNullSurfaceHandle) {
    LOG(ERROR) << "ContextResult::kFatalFailure: "
                  "WebNNInterface clients must render offscreen.";
    return ContextResult::kFatalFailure;
  }

  if (init_params.attribs.context_type != CONTEXT_TYPE_WEBNN) {
    LOG(ERROR) << "ContextResult::kFatalFailure: Incompatible creation attribs "
                  "used with WebNNDecoder";
    return ContextResult::kFatalFailure;
  }
  ...
  std::unique_ptr<webnn::WebNNDecoder> decoder(webnn::WebNNDecoder::Create(
      this, command_buffer_.get(), manager->shared_image_manager(),
      memory_tracker_.get(), manager->gpu_preferences()));
...
MemoryTracker* WebNNCommandBufferStub::GetContextGroupMemoryTracker() const {
  return nullptr;
}

void WebNNCommandBufferStub::OnSwapBuffers(uint64_t swap_id, uint32_t flags) {}
